...
contract BasePositionManager is IBasePositionManager, ReentrancyGuard, Governable {
    ...
    constructor(address _vault, address _router, address _shortsTracker, address _wklay, uint256 _depositFee) public {
        vault = _vault;
        router = _router;
        wklay = _wklay;
    ...
    receive() external payable {
        require(msg.sender == wklay, "BasePositionManager: invalid sender");
    }
    ...
    function _transferInKLAY() internal {
        if (msg.value != 0) {
            IWKLAY(wklay).deposit{value: msg.value}();
        }
    }

    function _transferOutKLAYWithGasLimitIgnoreFail(uint256 _amountOut, address payable _receiver) internal {
        IWKLAY(wklay).withdraw(_amountOut);

        // use `send` instead of `transfer` to not revert whole transaction in case KLAY transfer was failed
