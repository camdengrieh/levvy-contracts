...
contract ShortsTracker is Governable, IShortsTracker {
    ...
    function getRealisedPnl(
        ...
        IVault _vault = vault;
        (uint256 size /*uint256 collateral*/, , uint256 averagePrice, , , , , uint256 lastIncreasedTime) = _vault.getPosition(
            _account,
            _collateralToken,
            _indexToken,
            false
    ...
    function _getNextGlobalAveragePrice(
        ...
        uint256 nextAveragePrice = _nextPrice.mul(_nextSize).div(hasProfit ? _nextSize.sub(nextDelta) : _nextSize.add(nextDelta));
