...
import "../oracle/interfaces/IOracleFlags.sol";
import "../amm/interfaces/IClaimSwapPair.sol";
...
contract VaultPriceFeed is IVaultPriceFeed {
    ...
    function setOracleFlags(address _oracleFlags) external onlyGov {
        oracleFlags = _oracleFlags;
    }
    ...
    function setTokens(address _btc, address _eth, address _klay) external onlyGov {
        btc = _btc;
        eth = _eth;
        klay = _klay;
    }

    function setPairs(address _klayUsdt, address _ethKlay, address _btcKlay) external onlyGov {
        klayUsdt = _klayUsdt;
        ethKlay = _ethKlay;
        btcKlay = _btcKlay;
    }
    ...
    function getAmmPrice(address _token) public view override returns (uint256) {
        ...
        if (_token == eth) {
            uint256 price0 = getPairPrice(klayUsdt, true);
            // for ethKlay, reserve0: KLAY, reserve1: KLAY
            uint256 price1 = getPairPrice(ethKlay, true);
        ...
        if (_token == btc) {
            uint256 price0 = getPairPrice(klayUsdt, true);
            // for btcKlay, reserve0: BTC, reserve1: KLAY
            uint256 price1 = getPairPrice(btcKlay, true);
            // this calculation could overflow if (price0 / 10**30) * (price1 / 10**30) is more than 10**17
            return price0.mul(price1).div(PRICE_PRECISION);
        }
