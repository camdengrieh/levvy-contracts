...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    event Initialize(address router, address vault, address wklay, address usdl, uint256 minExecutionFee, uint256 minPurchaseTokenAmountUsd);
    ...
    function initialize(
        ...
        emit Initialize(_router, _vault, _wklay, _usdl, _minExecutionFee, _minPurchaseTokenAmountUsd);
    }

    receive() external payable {
        require(msg.sender == wklay, "OrderBook: invalid sender");
    }
    ...
    function createSwapOrder(
        ...
        // always need this call because of mandatory executionFee user has to transfer in KLAY
        _transferInKLAY();

        if (_shouldWrap) {
            require(_path[0] == wklay, "OrderBook: only wklay could be wrapped");
    ...
    function cancelSwapOrder(uint256 _orderIndex) public nonReentrant {
        ...
        if (order.path[0] == wklay) {
            _transferOutKLAY(order.executionFee.add(order.amountIn), msg.sender);
        } else {
            IERC20(order.path[0]).safeTransfer(msg.sender, order.amountIn);
            _transferOutKLAY(order.executionFee, msg.sender);
        }
    ...
    function validateSwapOrderPriceWithTriggerAboveThreshold(address[] memory _path, uint256 _triggerRatio) public view returns (bool) {
        ...
        // 1. USDL doesn't have a price feed so we need to calculate it based on redepmtion amount of a specific token
        // That's why USDL price in USD can vary depending on the redepmtion token
        // 2. In complex scenarios with path=[USDL, KLAY, BTC] we need to know how much KLAY we'll get for provided USDL
    ...
    function executeSwapOrder(address _account, uint256 _orderIndex, address payable _feeReceiver) external override nonReentrant {
        ...
        uint256 _amountOut;
        if (order.path[order.path.length - 1] == wklay && order.shouldUnwrap) {
            _amountOut = _swap(order.path, order.minOut, address(this));
            _transferOutKLAY(_amountOut, payable(order.account));
        } else {
            _amountOut = _swap(order.path, order.minOut, order.account);
        }

        // pay executor
        _transferOutKLAY(order.executionFee, _feeReceiver);
    ...
    function cancelIncreaseOrder(uint256 _orderIndex) public nonReentrant {
        ...
        if (order.purchaseToken == wklay) {
            _transferOutKLAY(order.executionFee.add(order.purchaseTokenAmount), msg.sender);
        } else {
    ...
    function executeDecreaseOrder(address _address, uint256 _orderIndex, address payable _feeReceiver) external override nonReentrant {
        ...
        if (order.collateralToken == wklay) {
            _transferOutKLAY(amountOut, payable(order.account));
        } else {
    ...
    function _transferInKLAY() private {
        if (msg.value != 0) {
            IWKLAY(wklay).deposit{value: msg.value}();
        }
    }

    function _transferOutKLAY(uint256 _amountOut, address payable _receiver) private {
        IWKLAY(wklay).withdraw(_amountOut);
