...
contract LevvyFloor is ReentrancyGuard, TokenManager {
    ...
    function setBackedSupply(uint256 _backedSupply) public onlyAdmin {
        require(_backedSupply > backedSupply, "LevvyFloor: invalid _backedSupply");
        backedSupply = _backedSupply;
    }

    function setMintMultiplier(uint256 _mintMultiplier) public onlyAdmin {
        require(_mintMultiplier > mintMultiplier, "LevvyFloor: invalid _mintMultiplier");
        mintMultiplier = _mintMultiplier;
    }

    // mint refers to increasing the circulating supply
    // the LEV tokens to be transferred out must be pre-transferred into this contract
    function mint(uint256 _amount, uint256 _maxCost, address _receiver) public onlyHandler nonReentrant returns (uint256) {
        require(_amount > 0, "LevvyFloor: invalid _amount");
        ...
        require(cost <= _maxCost, "LevvyFloor: _maxCost exceeded");
        ...
        IERC20(levvy).transfer(_receiver, _amount);
    ...
    function burn(uint256 _amount, uint256 _minOut, address _receiver) public onlyHandler nonReentrant returns (uint256) {
        ...
        require(amountOut >= _minOut, "LevvyFloor: insufficient amountOut");

        backedSupply = backedSupply.sub(_amount);

        IMintable(levvy).burn(msg.sender, _amount);
