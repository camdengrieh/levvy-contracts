...
contract Timelock is ITimelock {
    ...
    function updateUsdlSupply(uint256 usdlAmount) external onlyKeeperAndAbove {
        ...
        if (usdlAmount > balance) {
            uint256 mintAmount = usdlAmount.sub(balance);
            IUSDL(usdl).mint(levLpManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdlAmount);
            IUSDL(usdl).burn(levLpManager, burnAmount);
        }
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        ILevLpManager(levLpManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }

    function setLevLpCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        ILevLpManager(levLpManager).setCooldownDuration(_cooldownDuration);
    }
