...
contract EsLevvyBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esLevvy;

    constructor(address _esLevvy) public {
        admin = msg.sender;
        esLevvy = _esLevvy;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsLevvyBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esLevvy).transferFrom(msg.sender, _accounts[i], _amounts[i]);
